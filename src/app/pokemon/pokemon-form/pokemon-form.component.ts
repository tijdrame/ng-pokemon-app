...
export class PokemonFormComponent implements OnInit {
  ...
  selectType($event: Event, type: string): void {
    ...
    if (isChecked) {
      ...
      //sinon cad c decoche on enleve si deja present
  ...
  onSubmit() {
    if(this.isAddForm) this.pokemonService.addPokemon(this.pokemon).subscribe((pokemon)=> this.router.navigate(['pokemon', pokemon.id]));
    else
